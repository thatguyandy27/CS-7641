...
class ClusteringExperiment(experiments.BaseExperiment):
    ...
    def __init__(self, details, verbose=False):
        ...
        self._nn_learning_rates = [0.004, 0.008, 0.016]
        self._nn_reg = [10 ** -x for x in np.arange(-1, 9.01, 1)]
        # self._nn_alphas =
        self._nn_activation = ['relu', 'logistic']
    ...
    def __do_perform(self, custom_out=None, main_experiment=None):
        ...
        grid = {'km__n_clusters': self._clusters,
                'NN__alpha': self._nn_reg,
                'NN__activation': self._nn_activation,
                'NN__learning_rate_init': self._nn_learning_rates,
                'NN__hidden_layer_sizes': self._nn_arch
                }
        mlp = MLPClassifier(early_stopping=True, random_state=self._details.seed)
        ...
        grid = {'gmm__n_components': self._clusters,
                'NN__alpha': self._nn_reg,
                'NN__activation': self._nn_activation,
                'NN__learning_rate_init': self._nn_learning_rates,
                'NN__hidden_layer_sizes': self._nn_arch
